...
class MY_Controller extends CI_Controller {
    ...
    public function __construct()
    {
        ...
        $urls = $class.'/'.$method;
//        var_dump($urls);die();
        $astrict = array('Frame/main','index/index','index/login','frame/main');
        if($this->isLogin() && $class != 'App' && !in_array($urls,$astrict) ){
            if($this->session->userdata('uid') != 1){
                print_r('<script>alert("对不起，您没有权限查看改模块",history.back());</script>');
            }
        }
        $this->resetBaseUrl();
