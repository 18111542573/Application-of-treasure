...
class App extends MY_Controller {
    ...
    public function save() {
        ...
        $ios_path = $this->input->post('ios_path', true);
        ...
        if ($app_id > 0) {
            ...
            if (is_array($ios_path) && sizeof($ios_path) > 0) {
                foreach ($ios_path as $key => $apk) {
                    if (isset($ios_size[$key])) {
                        if (strpos($ios_size[$key], 'K') > 0) {
                            $size_app = intval(floatval($ios_size[$key]) * 1024);
                        } elseif (strpos($ios_size[$key], 'M') > 0) {
                            $size_app = intval(floatval($ios_size[$key]) * 1024 * 1024);
                        } else {
                            $size_app = $ios_size[$key];
                        }
                    }
                    $this->m_History->createHistory(array(
                        "app_id" => $app_id,
                        "history_version" => isset($ios_version[$key]) ? $ios_version[$key] : '',
                        "history_update_time" => time(),
                        "history_size" => isset($ios_size[$key]) ? $size_app : '',
                        "history_system" => isset($ios_system[$key]) ? $ios_system[$key] : '',
                        "data_history_id" => 0,
                        "history_type" => 2,
                        "history_minSdkVersion" => isset($ios_sdk[$key]) ? $ios_sdk[$key] : '',
                        "history_package" => isset($ios_package[$key]) ? $ios_package[$key] : '',
                        "history_VersionCode" => isset($ios_code[$key]) ? $ios_sdk[$key] : 0,
                        "history_permission" => isset($ios_permission[$key]) ? $ios_permission[$key] : '',
                        "history_file" => $apk
                    ));
                }
            }
        ...
        foreach ($ios_path as $key => $apk) {
            if (isset($ios_size[$key])) {
                if (strpos($ios_size[$key], 'K') > 0) {
                    $size_app1 = intval(floatval($ios_size[$key]) * 1024);
                } elseif (strpos($ios_size[$key], 'M') > 0) {
                    $size_app1 = intval(floatval($ios_size[$key]) * 1024 * 1024);
                } else {
                    $size_app1 = $ios_size[$key];
                }
            }
            $this->m_History->createHistory(array(
                "app_id" => $insert_id,
                "history_version" => isset($ios_version[$key]) ? $ios_version[$key] : '',
                "history_update_time" => time(),
                "history_size" => isset($ios_size[$key]) ? $size_app1 : '',
                "history_system" => isset($ios_system[$key]) ? $ios_system[$key] : '',
                "data_history_id" => 0,
                "history_type" => 2,
                "history_minSdkVersion" => isset($ios_sdk[$key]) ? $ios_sdk[$key] : '',
    ...
    public function update_history() {
        ...
        $history_size = strtolower($this->input->post('history_size'));
        if(substr($history_size, -1) == 'm'){
            $history_size = substr($history_size, 0, -1) * 1000000;
        }

        $data = array(
            'history_file' => $this->input->post('history_file'),
            'history_version' => $this->input->post('history_version'),
            'history_size' => $history_size,
            "history_update_time" => time(),
            'history_package' => $this->input->post('history_package'),
            'history_system' => $this->input->post('history_system'),
        );
        $this->m_History->updateHistoryById($data, $history_id);
        $this->output->ok_json("更新成功");
    }
