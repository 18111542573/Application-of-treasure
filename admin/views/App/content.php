...
<script language="javascript" type="text/javascript" src="<?php echo static_url('/js/editable.js'); ?>"></script>
                        ...
                        <td class="alignleft inline-uuploadifyploadify">
                            <div class="l">
                                <input type="text" class="ipt ml15" style="width:178px; margin-right:10px" name="app_logo" id="app_logo" value="<?php echo isset($info['app_logo']) ? $info['app_logo'] : ''; ?>"/>
                                    <a href="javascript:void(0);" class="but2"  id="upload_logo" style="margin:0 0 0 30px;font-size:15px;">添加</a>
                            </div>
                            <div class="l">
                                <span id="showimg">
...
<!-- box -->
            ...
            <table class="tb-2" border="1" bordercolor="#cee1ee">
            <tr>
                <td class="td-bg"  width="12%"><h2 style="font-size:14px; color:#333">添加Android远程应用链接</h2</td>
                <td class="alignleft">
                    <div class="l">
                ...
                <td class="td-bg"  width="12%"><h2 style="font-size:14px; color:#333">添加IOS远程应用链接</h2</td>
                <td class="alignleft">
                    <div class="l">
                        <input type="text" class="ipt ml15" id="remote_ios" value=""/>
                    </div>
                </td>
                <td class="alignleft" width="12%">
                    <div class="l">
                        <a href="javascript:void(0);" class="but2 l" onclick="url_ios3()" style="margin:0 0 0 30px;font-size:15px;">添加</a>
                    </div>
                    <div class="l"></div>
                </td>
<!--                <td class="td-bg"  width="10%" align="center"><h2 style="font-size:14px; color:#333">添加本地应用</h2</td>-->
<!--                <td class="alignleft" width="23%" >-->
<!--                    <div class="l">-->
<!--                        <a href="javascript:void(0);" class="but2 l"  id="upload_local_apk" style="margin:0 0 0 30px;font-size:15px;">添加</a>-->
<!--                    </div>-->
<!--                    <div class="l"></div>-->
<!--                </td>-->
...
<!-- /box -->
                    ...
                    <?php if (is_array($history) && sizeof($history) > 0) : ?>
                        <?php foreach ($history as $value) : ?>
                            <tr>
                                ...
                                <?php else : ?>
                                    <td width="100" editable><?php echo $value['history_file']; ?></td>
                                <?php endif; ?>
                                <td width="50" editable><?php echo $value['history_version']; ?></td>
                                <td width="50" editable><?php echo appSize($value['history_size']);?>M</td>
                                <td width="50" editable><?php echo $value['history_package']; ?></td>
                                <td width="50" editable><?php echo $value['history_system']; ?></td>
                                <td>
                                    <?php if (!empty($info['data_history_id']) || $info['data_app_id'] == 0) : ?>
                                        <a href="javascript:void(0)" onclick="update_history(this, <?php echo $value['history_id']; ?>)">更新</a>
                                        <a href="javascript:void(0)" onclick="delete_history(this, <?php echo $value['history_id']; ?>)">删除</a>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
            ...
            <table class="tb-2" border="1" bordercolor="#cee1ee">
                <tr>
                    ...
                    <td class="alignleft" width="23%">
                        <div class="l" >
                            <a href="javascript:void(0);" class="but2 l"  id="upload_local_image" style="margin:0 0 0 30px;font-size:15px;">添加</a>
                        </div>
                        <div class="l"></div>
                    </td>
...
<script>
    ...
    function update_history(obj, id) {
        var data = {"history_id": id};
        var index = ['history_file', 'history_version', 'history_size', 'history_package', 'history_system'];
        $(obj).parent("td").siblings().each(function (i,e) {
            data[index[i]] = $(e).text();
        });
        show_msg('您确认要更新此版本吗?', true, function () {
            $.post(build_url('App', 'update_history'), data, function (result) {
                if (!result) {
                    alert('远程主机无响应');
                    return false;
                }
    ...
    $(function () {
        ...
        $('#upload_logo').ajaxUploadPrompt({
            url: '<?=build_url("Index", "uploadLogo", array('time' => $time, 'token' => $token))?>',
            data: ":file",
        ...
        $('#upload_local_image').ajaxUploadPrompt({
            url: '<?=build_url("Index", "uploadImage", array('time' => $time, 'token' => $token))?>',
            data: ":file",
            success: function (data, status, xhr) {
                if (data.indexOf('Error') >= 0) {
                    show_msg(data.substring(6));
        ...
        $('#upload_local_apk').ajaxUploadPrompt({
            url: '<?=build_url("Index", "uploadApk", array('time' => $time, 'token' => $token))?>',
            data: ":file",
            success: function (data, status, xhr) {
                console.log(data);
                if (data.indexOf('Error') >= 0) {
    ...
    function url_ios3() {
        var apkUrl = $('#remote_ios').val();
        if(apkUrl.indexOf('http') === -1){
            show_msg('输入ios http地址');
            return false;
        }
        ...
        $.post(url,{'url':encodeURIComponent(apkUrl)},function(result){
            ...
            if(result.code === 0){
                ...
                var html = '';
                html += '<tr>';
                html += '    <td>' + data[0] + '<input type="hidden" name="ios_path[]" value="' + data[0] + '" /></td>';
                html += '    <td>' + data[1] + '<input type="hidden" name="ios_version[]" value="' + data[1] + '" /></td>';
                html += '    <td>' + size.toFixed(2) + 'MB <input type="hidden" name="ios_size[]" value="' + data[2] + '" /></td>';
                html += '    <td>' + data[3] + '<input type="hidden" name="ios_package[]" value="' + data[3] + '" /></td>';
                html += '    <td>' + data[4] + '<input type="hidden" name="ios_system[]" value="' + data[4] + '" /></td>';
                html += '    <td>';
                html += '        <input type="hidden" name="ios_code[]" value="' + data[5] + '" />';
                html += '        <input type="hidden" name="ios_sdk[]" value="' + data[6] + '" />';
                html += '        <input type="hidden" name="ios_permission[]" value="' + data[7] + '" />';
                html += '        <a href="javascript:;" onclick="remove_line(this)">删除</a>';
                html += '    </td>';
                html += '</tr>';
                $('#history_list').append(html);
